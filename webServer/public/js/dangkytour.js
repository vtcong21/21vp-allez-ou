...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var modalTongCong = document.getElementById('tongTien');
    // U code
    var modalNganHang = document.getElementById('soTien');
    // U het code
    ...
    function updateTotalDisplay() {
        ...
        modalNganHang.textContent = total.toLocaleString();
    }
...
function validateAndSaveOrder() {
    ...
    const tickets = getFormData(); // Hãy chắc chắn bạn có một hàm getFormData() để lấy thông tin vé
    const totalPrice = tickets.reduce((total, ticket) => total + ticket.price, 0);
    const item = {
        _id: itemId,
        tourCode: tourData.code,
...
function showModal3() {
    const emaiUser = document.getElementById('emailUser').textContent;
    const otp = document.getElementById('input-otp').value;
    console.log(otp);
    verifyOTP(emaiUser, otp);
}
...
async function verifyOTP(email, otp) {
    try {
        Swal.showLoading();
        console.log(otp);
        const response = await axios.post('/user/checkVerifyOTP', { 
            email: email,
